...
export default function ProductsPage() {
  ...
  useEffect(() => {
    fetch('/api/auth/me').then(r => r.ok ? r.json() : null).then(d => {
      if (d) setCanManage(d.role === 'ADMIN' || d.permissions?.includes('manage-products'))
    })
  }, [])
        ...
        {loading ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-600 text-xs uppercase tracking-wider">
              <tr>
                {['Name', 'SKU', 'Cost Price', 'Selling Price', 'Stock', 'Alert At', ''].map(h => (
              ...
              {filtered.map(p => {
                ...
                return (
                  <tr key={p.id} className={lowStock ? 'bg-amber-50' : 'hover:bg-gray-50'}>
                    ...
                    <td className="px-4 py-3 text-gray-500 text-xs">≤ {p.lowStockThreshold}</td>
                    <td className="px-4 py-3">
                      {canManage && (
